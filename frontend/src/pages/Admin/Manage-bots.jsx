        ...
        const fetchData = async () => {
            try {
                ...
                const mappedBots = Array.isArray(boatsRes) ? boatsRes.map((b) => {
                    ...
                    return {
                        id: boatMap[b.id] || `Boat-${b.id}`,
                        status: b.is_active ? 'Active' : 'Offline',
                        battery: b.battery_level ?? 0,
                        assignedLocation: 'None',
                        assignedOperator: op ? op.name : 'None',
                        ...
                        runtimeToday: '0h 0m',
                        totalTrash: '0 bags',
                        archived: !b.is_active,
                    };
                }) : [];

                const mappedOperators = Array.isArray(operatorsRes) ? operatorsRes.map((o) => ({
                    id: `OP-${String(o.id).padStart(3, '0')}`,
                    name: o.name,
                    status: o.status,
                    assignedBot: o.assigned_bot ? (boatMap[o.assigned_bot] || `Boat-${o.assigned_bot}`) : null,
